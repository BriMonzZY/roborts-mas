...
void DynamicObstacleLayer::OnInitialize()
{
  ...
  // debug
  // std::cout << observation_keep_time << " " << expected_update_rate << " " << transform_tolerance << " " << max_obstacle_height << " " << min_obstacle_height << " " << obstacle_range << std::endl;
  // std::cout << topic_string << " " << sensor_frame << " " << obstacle_size_ << std::endl;
...
/**
 * callback of topic:goal_sub_  ("/global_planner_node_action/goal")
*/
                                      ...
                                      const std::shared_ptr<ObservationBuffer> &buffer)
{
  ...
  // 判断目标点是不是 (4.0, 0.8) or (4.0, 4.2)
  std::cout << "goal pose x : " << goal.pose.position.x << "," << "goal pose y : " << goal.pose.position.y << std::endl;
  if (is_update) {
  ...
  ec.setClusterTolerance (0.4);  // 设置近邻搜索的搜索半径
  ec.setMinClusterSize (10);  // 设置最小聚类尺寸
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
    ...
    // 筛选对应的条件的动态障碍物加入costmap
    if (set_point) {
      vec_obstacle_center.push_back(center);  // 将中心加入vecto中，等待加入costmap
    }
    else {
  ...
  // 将新的边界加入costmap中 (以动态障碍物中心点为圆心的圆)
