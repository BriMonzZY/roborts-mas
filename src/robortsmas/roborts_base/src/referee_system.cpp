...
void RefereeSystem::RobotStatusCallback(const std::shared_ptr<roborts_sdk::cmd_game_robot_status> raw_robot_status){
  ...
  if ( raw_robot_status->robot_id != 7 &&
       raw_robot_status->robot_id != 107 ){

    ROS_WARN("For RMUL, "
             "please set robot id to Blue7 or Red7 in the referee system main control module, "
...
void RefereeSystem::RobotDamageCallback(const std::shared_ptr<roborts_sdk::cmd_robot_hurt> raw_robot_damage){
  ...
  ROS_WARN("damage_type : %d", robot_damage.damage_type);
  if(robot_damage.damage_type == 0x0) {
    ROS_ERROR("hurt armnor id : %d", robot_damage.damage_source);
  }
