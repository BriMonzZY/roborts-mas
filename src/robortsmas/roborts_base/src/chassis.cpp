...
void Chassis::ChassisInfoCallback(const std::shared_ptr<roborts_sdk::cmd_chassis_info> chassis_info)
{
  ...
  // geometry_msgs::Quaternion q = tf::createQuaternionMsgFromYaw(chassis_info->gyro_angle / 1800.0 * M_PI - 90/180.0*M_PI);
  ...
  // odom_.twist.twist.angular.z = chassis_info->gyro_rate / 1800.0 * M_PI - 90/180.0*M_PI;
