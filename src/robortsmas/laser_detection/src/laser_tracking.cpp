...
/**
 * callback of shooter_camera_sub_
*/
 ...
 * 更新机器人的信息
*/
void LaserTracking::UpdateRoboState(void)
{
  tf::StampedTransform transform_pose_;
  try {
    // 得到map到base_link的坐标变换
...
/**
 * callback of camera_armors_sub_
 * 维护着 camera_history_buffer_ 列表
*/
...
void LaserTracking::FusionLaserCamera(std::vector<TrackingPoint> laser_enemies, std::vector<Point> camera_enemies)
{
  ...
  if(false) {
      ...
      if(updated_robo_state_){
        ...
        if(isLaserInCameraView(laser_enemies[i])) {
          ...
          if(camera_enemies.size()>0) {
            // find near camera 计算所有相机找到的与激光雷达找到的最小距离
            ...
            // 如果最小距离大于相机与激光雷达判断为同一个的阈值判断为已经阵亡
            if(near_distance > camer_laser_thresh_) {
              // update died car
        ...
        } else { // not in camera view 不在相机视野内判断为存活
    ...
    for(int i=0; i<camera_enemies.size(); i++) {
      RemoveDiedCar(camera_enemies[i]);  // 将相机检测到的从阵亡队列里删除
  ...
  // update active car by died car  再次判断和died_cars的最小距离大于阈值，更新active_cars
  ...
  PublishActivePoint(update_active_cars);  // 发布active_cars的位置信息 (roborts_msgs::ArmorsPos)
}
